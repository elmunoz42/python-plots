...
def mse(theta):
    ...
    '''
    return np.mean((credit['Balance']-theta*credit['Rating'])**2)
...
step_size = 0.1
range_number = 200
def gradient_descent(xs_array, step_size, range_number):
    for _ in range(range_number):
        x0 = xs[-1]
        xs_array.append(x0 - step_size * df(x0))
    return xs_array
    
xs = gradient_descent(xs, step_size, range_number)
theta_big_step = np.float64(xs[-1])
print(f"theta_big_step: {theta_big_step}")
### ANSWER CHECK
xs[-5:]
...
theta_small_step = np.float64(xs[-1]) 
...
error = np.absolute(lr.coef_ - theta_small_step)
